...
function SA = suffixArray(s, n, K)
    ...
    for k = 0:(n-1)
        ...
        if ( (SA12(1+t) <  n0) && leq1(s(1+i), s12(1+SA12(1+t) + n0), s(1+j), s12(1+floor(j/3))) ) || ((SA12(1+t) >= n0) && leq2(s(1+i),s(1+i+1),s12(1+SA12(1+t)-n0+1), s(1+j),s(1+j+1),s12(1+j/3+n0))) 
			SA(1+k) = i;
			t = t+1;
            if (t == n02)
				ktmp = k+1;
            ...
            if (p == n0) 
                ...
                while(t < n02)
                    if (SA12(1+t) < n0)
						SA(1+ktmp) = SA12(1+t) * 3 + 1;
                    else
						SA(1+ktmp) = (SA12(1+t) - n0) * 3 + 2;
                    end
					t = t+1;
					ktmp = ktmp+1;
                end
